...
xls = XlsImporterAllBase.new("db/Nota Fiscal cpf 0464.xlsx", 1)
names = xls.unknown
names.each_key { |contribuinte|
  @user = User.new
  @user.email = Faker::Internet.email
  @user.password = '123456'
  @user.cpf = contribuinte
  @user.name = names[contribuinte]
  @user.save!
}
puts "Criados #{names.length} USERs:"
p names
...
User.all.each { |user|
  rand(3..10).times {
    ...
    @pedido.situacao = ['pendente', 'disponível'].sample
    @pedido.save!
  }
}
puts 'Criados pedidos'

Pedido.all.where(situacao: 'pendente').each { |pedido|
...
User.all.each { |user|
  rand(3..10).times {
    ...
    @pedido.situacao = ['pendente', 'disponível', 'concluída'].sample
    @pedido.save
  }
}
puts 'criados pedidos'
